...
function email() {
  $.ajax({
    ...
    success: function (response) {
      let card = response["card"];
      for (let i = 0; i < card.length; i++) {
        let email = card[i]["name"];
        let num = card[i]["num"];
        let subject = card[i]["subject"];
        let message = card[i]["message"];
        ...
        let temp_html = `
          <tr>
                                        ...
                                        <td>${subject}  <br> ${message}</td> 
                                        <td>
                                          <button class="btn btn-danger mb-2" onclick="deleteemail('${num}')">
                                              <i class="fas fa-dumpster fa-sm"></i>
                                          </button></td>
                                      </tr>
          `;
